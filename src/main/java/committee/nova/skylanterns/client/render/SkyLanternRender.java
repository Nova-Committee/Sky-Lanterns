...
public class SkyLanternRender extends EntityRenderer<SkyLanternEntity> {
    ...
    public SkyLanternRender(EntityRendererProvider.Context context) {
        ...
        TEXTURES.put(DyeColor.ORANGE.getId(), new ResourceLocation(SkyLanterns.MOD_ID + ":textures/entity/sky_lantern_orange.png"));
        TEXTURES.put(DyeColor.PINK.getId(), new ResourceLocation(SkyLanterns.MOD_ID + ":textures/entity/sky_lantern_pink.png"));
    }

    @Override
    public @NotNull ResourceLocation getTextureLocation(SkyLanternEntity entity) {
        return new ResourceLocation(SkyLanterns.MOD_ID + ":textures/entity/sky_lantern_" + entity.getColor().getRegistryPrefix() + ".png");
    }
    ...
    public void render(SkyLanternEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if (ModConfig.COMMON.enableLanternShader.get()) {
            ...
            float cr = pEntity.getColor().getColor(0);
            float cg = pEntity.getColor().getColor(1);
            float cb = pEntity.getColor().getColor(2);
            float ca = 0.9F;
            glowConsumer.vertex(matrix, -s, -s, 0).color(cr, cg, cb, ca).uv(0.0F, 1.0F).endVertex();
            glowConsumer.vertex(matrix, s, -s, 0).color(cr, cg, cb, ca).uv(1.0F, 1.0F).endVertex();
            glowConsumer.vertex(matrix, s, s, 0).color(cr, cg, cb, ca).uv(1.0F, 0.0F).endVertex();
            glowConsumer.vertex(matrix, -s, s, 0).color(cr, cg, cb, ca).uv(0.0F, 0.0F).endVertex();
            pPoseStack.popPose();
        }
    ...
    private <E extends Entity> void renderLeash(SkyLanternEntity pEntityLiving, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, E pLeashHolder) {
        ...
        double f4 = Mth.invSqrt(f * f + f2 * f2) * 0.025F / 2.0F;
        float f5 = (float) (f2 * f4);
        float f6 = (float) (f * f4);
